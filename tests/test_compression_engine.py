...
def test_auralite_compression():
    ...
    for text in test_cases:
        ...
        print(
            f"✅ Compressed '{text[:30]}...' → {len(compressed)} bytes (ratio: {metadata['ratio']:.2f}x)"
        )
...
def test_pattern_semantic_compression():
    ...
    print(
        f"✅ Pattern Semantic compressed {metadata['original_size']} → {metadata['compressed_size']} bytes"
    )
    print(f"   - Ratio: {metadata['ratio']:.2f}x")
...
def test_compression_method_detection():
    ...
    test_cases = [
        (
            bytes([CompressionMethod.BINARY_SEMANTIC.value, 0x00, 0x01]),
        ...
        (
            bytes([CompressionMethod.PATTERN_SEMANTIC.value, 0x00, 0x01]),
