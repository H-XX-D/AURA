...
def test_parse_json_format_simple():
    ...
    json_text = json.dumps(
        {"function": "execute_task", "args": {"task_id": "123", "name": "test_task"}}
    )
...
def test_parse_json_format_with_arguments_key():
    ...
    json_text = json.dumps(
        {"function": "query_database", "arguments": {"table": "users", "limit": 10}}
    )
...
def test_register_function():
    ...
    parser.register_function(
        function_name="custom_function", function_id=100, routing_hint="custom_handler"
    )
...
def test_orchestrator_route_from_metadata_non_function():
    ...
    metadata = {"type": "regular_message", "size": 1024}
...
def test_orchestrator_dispatch_no_handler():
    ...
    call = FunctionCall(
        function_name="execute_task", arguments={}, function_id=1, routing_hint="task_executor"
    )
...
def test_orchestrator_process_message_slow_path():
    ...
    metadata = {"type": "regular_message", "size": 100}

    text = "query_database(table=users, limit=10)"

    used_fast_path, result = orchestrator.process_message(
        text=text, compressed_data=b"", metadata=metadata
    )
...
def test_end_to_end_workflow():
    ...
    used_fast_path, result = orchestrator.process_message(
        text=text, compressed_data=b"compressed_payload", metadata=metadata
    )
