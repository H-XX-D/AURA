...
def test_worker_initialization_platform_mode():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        print(
            f"   - Template ID range: {worker.discovery_engine.starting_template_id}-{worker.discovery_engine.max_template_id}"
        )
...
def test_cold_storage():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        print(
            f"   - Template {template_id}: {worker2.discovery_engine.cold_storage[template_id].pattern}"
        )
