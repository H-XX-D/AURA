...
def test_audit_database_initialization():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        expected_columns = [
            "id", "timestamp", "event_id", "level", "event_type",
            "user_id", "session_id", "source_ip", "method",
            "original_size", "compressed_size", "compression_ratio",
            "latency_ms", "data_hash", "result_hash", "metadata",
...
def test_insert_and_query_operations():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert "metadata" in first_result, f"Missing metadata key. Keys: {list(first_result.keys())}"
...
def test_aggregate_metrics():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        print(f"✅ BRIO metrics: {brio_metrics['operations']} ops, "
        ...
        print(f"✅ BRIO_FULL metrics: {brio_full_metrics['operations']} ops, "
...
def test_compression_auditor_facade():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert actual_compress_hash == expected_compress_hash, \
            f"Compress data hash mismatch: expected {expected_compress_hash[:16]}, got {actual_compress_hash[:16]}"
        assert actual_decompress_hash == expected_decompress_hash, \
...
def test_hash_chain_integrity():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for i in range(1, len(rows)):
            ...
            assert current_prev_hash == prev_entry_hash, \
...
def test_aggregate_metrics_via_auditor():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for method in methods:
            ...
            print(f"   - {method}: {metrics[method]['operations']} ops, "
