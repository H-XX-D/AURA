...
def test_import_compatibility():
    ...
    from aura_compression.compressor_refactored import ProductionHybridCompressor as RefactoredCompressor
...
def test_backwards_property():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        print(f"   - Points to template_library: {compressor.templates is compressor.template_library}")
...
def test_edge_cases():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        print(f"   ✓ Very long string: {len(long_text)} → {len(compressed)} bytes (ratio: {metadata['ratio']:.2f}x)")
