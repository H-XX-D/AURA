...
def test_compress_medium_text():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        text = (
            "This is a medium-length test message that should be compressed using AURALITE or BRIO."
        )
        compressed, method, metadata = compressor.compress(text)
...
def test_fast_path_compression():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        compressed, method, metadata = compressor.compress(
            text, template_id=template_id, slots=["All systems operational"]
        )
...
def test_decompress_binary_legacy():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        compressed, method, metadata = compressor.compress(
            text, template_id=template_id, slots=["server-01", "10:30 AM"]
        )
...
def test_template_healing():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            compressed, method, metadata = compressor.compress(
                text, template_id=999, slots=["Test"]  # Non-existent
            )
