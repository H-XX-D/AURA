...
class NetworkSimulator:
    ...
    def __init__(self, duration_seconds: int = 60, enable_scorer: Optional[bool] = None):
        ...
        print("Template discovery worker configured (background mode - not started during simulation)")
        ...
        self.message_templates = {
            ...
            'repetitive_text': [
                ...
                'System status: OK | Memory: {memory}% | CPU: {cpu}% | Network: {network}Mbps ' * random.randint(1, 5)
            ]
        }
    ...
    def generate_message(self, message_type: str) -> str:
        ...
        replacements = {
            ...
            'error_message': random.choice(['Connection timeout', 'Invalid credentials', 'Server error']),
            ...
            'base64_data': ''.join(random.choices('ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/', k=100)),
        ...
        if message_type == 'api_request':
            ...
            if size_multiplier > 0.6:
                ...
                stack_trace = '\n'.join([f'  at function_{i} (file_{i}.js:{random.randint(1, 100)})' for i in range(stack_lines)])
                message += f'\nStack trace:\n{stack_trace}'
            ...
            if size_multiplier > 0.5:
                ...
                replacements['base64_data'] = ''.join(random.choices('ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/', k=data_size))
                message = template
                for key, value in replacements.items():
                    message = message.replace(f'{{{key}}}', str(value))
            elif size_multiplier < 0.3:
    ...
    def process_message(self, message: str, message_type: str) -> Dict[str, Any]:
        ...
        try:
            ...
            bandwidth_bps = len(message) / processing_time if processing_time > 0 else 0  # Bytes per second
        ...
        while datetime.now() < self.end_time and self.running:
            try:
                ...
                message_type = random.choices(
                    list(message_type_weights.keys()),
                    weights=list(message_type_weights.values())
                )[0]
    ...
    def generate_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            'compression_performance': {
                ...
                'average_compression_ratio': statistics.mean(self.stats['compression_ratios']) if self.stats['compression_ratios'] else 0,
                'median_compression_ratio': statistics.median(self.stats['compression_ratios']) if self.stats['compression_ratios'] else 0,
                'min_compression_ratio': min(self.stats['compression_ratios']) if self.stats['compression_ratios'] else 0,
                'max_compression_ratio': max(self.stats['compression_ratios']) if self.stats['compression_ratios'] else 0,
                'compression_ratio_stddev': statistics.stdev(self.stats['compression_ratios']) if len(self.stats['compression_ratios']) > 1 else 0
            },
            'message_type_distribution': dict(self.stats['message_types']),
            'template_discovery': {
                'template_matches': self.stats['template_matches'],
                'template_match_rate': self.stats['template_matches'] / max(self.stats['total_messages'], 1),
            ...
            'performance_metrics': {
                ...
                'median_processing_time': statistics.median(self.stats['processing_times']) if self.stats['processing_times'] else 0,
                'messages_per_second': self.stats['total_messages'] / actual_duration.total_seconds(),
                'processing_time_stddev': statistics.stdev(self.stats['processing_times']) if len(self.stats['processing_times']) > 1 else 0,
                'average_message_size': statistics.mean(self.stats['message_sizes']) if self.stats['message_sizes'] else 0,
                'median_message_size': statistics.median(self.stats['message_sizes']) if self.stats['message_sizes'] else 0,
                'min_message_size': min(self.stats['message_sizes']) if self.stats['message_sizes'] else 0,
                'max_message_size': max(self.stats['message_sizes']) if self.stats['message_sizes'] else 0,
                'size_distribution': self.stats['size_distribution'],
                'latency_metrics': {
                    'average_latency_ms': statistics.mean(self.stats['latency_stats']) if self.stats['latency_stats'] else 0,
                    'median_latency_ms': statistics.median(self.stats['latency_stats']) if self.stats['latency_stats'] else 0,
                    'min_latency_ms': min(self.stats['latency_stats']) if self.stats['latency_stats'] else 0,
                    'max_latency_ms': max(self.stats['latency_stats']) if self.stats['latency_stats'] else 0,
                    'latency_stddev_ms': statistics.stdev(self.stats['latency_stats']) if len(self.stats['latency_stats']) > 1 else 0,
                    'p95_latency_ms': statistics.quantiles(self.stats['latency_stats'], n=20)[18] if len(self.stats['latency_stats']) >= 20 else max(self.stats['latency_stats']) if self.stats['latency_stats'] else 0
                },
                'bandwidth_metrics': {
                    'average_bandwidth_bps': statistics.mean(self.stats['bandwidth_stats']) if self.stats['bandwidth_stats'] else 0,
                    'median_bandwidth_bps': statistics.median(self.stats['bandwidth_stats']) if self.stats['bandwidth_stats'] else 0,
                    'min_bandwidth_bps': min(self.stats['bandwidth_stats']) if self.stats['bandwidth_stats'] else 0,
                    'max_bandwidth_bps': max(self.stats['bandwidth_stats']) if self.stats['bandwidth_stats'] else 0,
                    'bandwidth_stddev_bps': statistics.stdev(self.stats['bandwidth_stats']) if len(self.stats['bandwidth_stats']) > 1 else 0,
                    'total_data_processed_mb': sum(self.stats['message_sizes']) / (1024 * 1024) if self.stats['message_sizes'] else 0,
                    'effective_bandwidth_mbps': (sum(self.stats['message_sizes']) / (1024 * 1024)) / (actual_duration.total_seconds() / 60) if actual_duration.total_seconds() > 0 else 0
                }
            },
            'error_analysis': {
                'total_errors': len(self.stats['errors']),
                'error_rate': len(self.stats['errors']) / max(self.stats['total_messages'] + len(self.stats['errors']), 1),
    ...
    def _generate_detailed_analysis(self) -> Dict[str, Any]:
        ...
        analysis = {
            'template_matching_analysis': {
                ...
                'failure_rate': self.stats['template_matching_failures'] / max(self.stats['template_matching_attempts'], 1),
                'success_rate': (self.stats['template_matching_attempts'] - self.stats['template_matching_failures']) / max(self.stats['template_matching_attempts'], 1),
    ...
    def _analyze_method_usage(self) -> Dict[str, Any]:
        ...
        for method, details in self.stats['compression_method_details'].items():
            ...
            breakdown[method] = {
                ...
                'avg_available_methods': statistics.mean([d['available_methods'] for d in details]) if details else 0
            }
    ...
    def _analyze_method_effectiveness(self) -> Dict[str, Any]:
        ...
        for method, details in self.stats['compression_method_details'].items():
            ...
            effectiveness[method] = {
                ...
                'usage_efficiency': len([r for r in ratios if r > 1.1]) / len(ratios),  # Good compression (>10% better)
    ...
    def _calculate_tradeoff_score(self, ratios: List[float], latencies: List[float]) -> float:
        ...
        normalized_latencies = [(max_lat - l) / (max_lat - min_lat) for l in latencies]  # Invert latency (lower is better)
        
        # Tradeoff score: weighted average favoring compression ratio slightly more
        tradeoff_scores = [0.6 * r + 0.4 * l for r, l in zip(normalized_ratios, normalized_latencies)]
        return statistics.mean(tradeoff_scores)

    def _calculate_bandwidth_efficiency(self, ratios: List[float], bandwidths: List[float]) -> float:
        ...
        efficiencies = [ratio * (bandwidth / 1000) for ratio, bandwidth in zip(ratios, bandwidths)]  # Normalize bandwidth to KB/s
...
def main(argv: Optional[List[str]] = None):
    ...
    try:
        ...
        if scorer_status:
            ...
            recommendation = scorer_status.get('recommendation') or scorer_status.get('disabled_reason')
            if recommendation:
                print(f"Scorer Recommendation: {recommendation}")
        print(f"Average Message Size: {summary['performance_metrics']['average_message_size']:.0f} chars")
        print(f"Message Size Range: {summary['performance_metrics']['min_message_size']:.0f} - {summary['performance_metrics']['max_message_size']:.0f} chars")
        ...
        print(f"  Latency Range: {latency['min_latency_ms']:.2f} - {latency['max_latency_ms']:.2f} ms")
