...
def test_compression_validation():
    ...
    print(f"✅ AURALITE: {metadata_al['original_size']} → {metadata_al['compressed_size']} bytes (ratio: {metadata_al['ratio']:.2f}x)")
    ...
    print(f"✅ BRIO: {metadata_br['original_size']} → {metadata_br['compressed_size']} bytes (ratio: {metadata_br['ratio']:.2f}x)")
    ...
    is_valid_invalid = manager.validate_compression_result(text, invalid_data, {'method': 'unknown'})
...
def test_edge_cases():
    ...
    # Very long text
    long_text = "x" * 10000
    compressed_long, meta_long = manager.compress_with_method(
        long_text,
    ...
    print(f"✅ Unicode text handled correctly: {meta_uni['original_size']} → {meta_uni['compressed_size']} bytes")
