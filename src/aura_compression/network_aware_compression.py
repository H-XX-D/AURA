...
class NetworkAwareCompressor:
    ...
    def _compress_with_strategy(self, message: str, compressor: Any,
        ...
        # Force specific compression method based on strategy
        if config['target_ratio'] >= 3.0:
            # Maximum compression - try all methods
            forced_method = None
        elif config['target_ratio'] >= 2.0:
            # Balanced - prefer AURA_LITE over slower methods
            forced_method = CompressionMethod.AURA_LITE
        elif config['target_ratio'] >= 1.5:
            # Fast compression - use binary semantic or uncompressed
            forced_method = CompressionMethod.BINARY_SEMANTIC
        else:
            # Minimal compression - uncompressed
            forced_method = CompressionMethod.UNCOMPRESSED

        if forced_method:
            try:
                compressed, metadata = compressor._compression_engine.compress_uncompressed(message)
                if forced_method == CompressionMethod.UNCOMPRESSED:
                    method = type('MockMethod', (), {'name': 'UNCOMPRESSED'})()
                    metadata = {
                        'original_size': len(message.encode('utf-8')),
                        'compressed_size': len(compressed),
                ...
                elif forced_method == CompressionMethod.BINARY_SEMANTIC:
                    # Try binary semantic compression
                    template_match = compressor._find_template_match(message)
                    if template_match:
                        compressed, metadata = compressor._compression_engine.compress_binary_semantic(message, template_match)
                        method = type('MockMethod', (), {'name': 'BINARY_SEMANTIC'})()
                    else:
                        # Fall back to uncompressed if no template match
                        compressed, metadata = compressor._compression_engine.compress_uncompressed(message)
                        method = type('MockMethod', (), {'name': 'UNCOMPRESSED'})()
                elif forced_method == CompressionMethod.AURA_LITE:
                    compressed, metadata = compressor._compression_engine.compress_aura_lite(message)
                    method = type('MockMethod', (), {'name': 'AURA_LITE'})()
                else:
            ...
            # Use full compression selection
            compressed, method, metadata = compressor.compress(message)
